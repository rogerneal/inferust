...
use inferust::regression::{ElasticNet, Fgls, Lasso, Ols, QuantileRegression, RecursiveOls, Ridge, RollingOls, Wls};
...
fn mat(s: &mut u64, rows: usize, cols: usize) -> Vec<Vec<f64>> {
    (0..rows).map(|_| (0..cols).map(|_| norm(s)).collect()).collect()
}
...
fn count_y(x: &[Vec<f64>], beta: &[f64], offset: f64, s: &mut u64) -> Vec<f64> {
    x.iter()
        .map(|r| {
            ...
            let mut cdf = p;
            let u = lcg(s);
            while cdf < u && k < 2000 { k += 1; p *= mu / k as f64; cdf += p; }
            k as f64
        })
        .collect()
}
...
fn main() {
    ...
    // ── OLS ──────────────────────────────────────────────────────────────────
    {
        ...
        let m = Ols::new();
        bench("ols", n, r20, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── WLS ──────────────────────────────────────────────────────────────────
    {
        ...
        let m = Wls::new();
        bench("wls", n, r20, w3, || { m.fit(&x, &y, &w).unwrap(); });
    }
    ...
    // ── Ridge ─────────────────────────────────────────────────────────────────
    {
        ...
        let y = linear_y(&x, &b, 1.0, &mut s);
        let m = Ridge::new(0.1);
        bench("ridge", n, r20, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── Lasso ────────────────────────────────────────────────────────────────
    {
        ...
        bench("lasso", n, r20, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── ElasticNet ───────────────────────────────────────────────────────────
    {
        ...
        bench("elastic_net", n, r20, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── QuantileRegression ───────────────────────────────────────────────────
    {
        ...
        bench("quantile_reg", n, r20, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── RollingOls ───────────────────────────────────────────────────────────
    {
        ...
        bench("rolling_ols", n, r10, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── RecursiveOls ─────────────────────────────────────────────────────────
    {
        ...
        bench("recursive_ols", n, r10, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── FGLS (Cochrane-Orcutt) ───────────────────────────────────────────────
    {
        ...
        let y = linear_y(&x, &b, 1.0, &mut s);
        let m = Fgls::new();
        bench("fgls", n, r10, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── RobustLinearModel ────────────────────────────────────────────────────
    {
        ...
        bench("robust_lm", n, r10, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── Logistic GLM ─────────────────────────────────────────────────────────
    {
        ...
        let m = Logistic::new();
        bench("logistic", n, r20, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── Poisson GLM ──────────────────────────────────────────────────────────
    {
        ...
        let m = Poisson::new();
        bench("poisson", n, r20, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── Gamma GLM ────────────────────────────────────────────────────────────
    {
        ...
        bench("gamma_glm", n, r20, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── Probit ───────────────────────────────────────────────────────────────
    {
        ...
        let y = binary_y(&x, &b, &mut s);
        let m = Probit::new();
        bench("probit", n, r20, w3, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── NegativeBinomial ─────────────────────────────────────────────────────
    {
        ...
        bench("neg_binomial", n, r10, w1, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── MultinomialLogit ─────────────────────────────────────────────────────
    {
        ...
        let y: Vec<usize> = x.iter().map(|r| {
            ...
            let u = lcg(&mut s);
            if u < p[0] { 0 } else if u < p[0]+p[1] { 1 } else { 2 }
        }).collect();
        let m = MultinomialLogit::new();
        bench("mnlogit", n, r10, w1, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── OrderedLogit ─────────────────────────────────────────────────────────
    {
        ...
        let y: Vec<usize> = x.iter().map(|r| {
            ...
            if eta < -0.5 { 0 } else if eta < 0.5 { 1 } else { 2 }
        }).collect();
        let m = OrderedLogit::new();
        bench("ordered_logit", n, r5, w1, || { m.fit(&x, &y).unwrap(); });
    }
    ...
    // ── ZeroInflatedPoisson ──────────────────────────────────────────────────
    {
        ...
        let x = mat(&mut s, n, 2);
        let y: Vec<f64> = x.iter().map(|r| {
            if lcg(&mut s) < 0.2 { 0.0 } else {
                let mu = (r.iter().zip(b.iter()).map(|(xi,bi)| xi*bi).sum::<f64>() + 1.0).exp();
                count_y(&[r.clone()], &[1.0], mu.ln(), &mut { lcg(&mut s) as u64 })[0]
            }
        }).collect();
        let infl_x: Vec<Vec<f64>> = vec![vec![]; n];
        let m = ZeroInflatedPoisson::new();
        bench("zip", n, r5, w1, || { m.fit(&x, &y, &infl_x).unwrap(); });
    }
    ...
    // ── GEE ──────────────────────────────────────────────────────────────────
    {
        ...
        let m = Gee::new(GeeFamily::Poisson).with_working_correlation(WorkingCorrelation::Exchangeable);
        bench("gee", n, r5, w1, || { m.fit(&x, &y, &clusters).unwrap(); });
    }
    ...
    // ── KaplanMeier ──────────────────────────────────────────────────────────
    {
        ...
        let events: Vec<usize> = (0..n).map(|_| if lcg(&mut s) < 0.8 { 1 } else { 0 }).collect();
        let m = KaplanMeier::new();
        bench("kaplan_meier", n, r20, w3, || { m.fit(&times, &events).unwrap(); });
    }
    ...
    // ── CoxPH ────────────────────────────────────────────────────────────────
    {
        ...
        let b = [0.5,-0.3,0.8];
        let x = mat(&mut s, n, 3);
        let (times, events): (Vec<f64>, Vec<usize>) = x.iter().map(|r| {
        ...
        let m = CoxPh::new();
        bench("cox_ph", n, r5, w1, || { m.fit(&times, &events, &x).unwrap(); });
    }
    ...
    // ── t-test (two-sample) ──────────────────────────────────────────────────
    {
        ...
        bench("ttest_2samp", n, r20, w3, || { ttest::two_sample(&a, &b).unwrap(); });
    }
    ...
    // ── Mann-Whitney U ───────────────────────────────────────────────────────
    {
        ...
        bench("mann_whitney", 1000, r20, w3, || { nonparametric::mann_whitney(&a, &b).unwrap(); });
    }
}
